...
{
  if (polling == true) FrSkySportDecoder(new FrSkySportPollingDynamic());
  ...
  if (serial.port != NULL)
  {
    ...
    if (serial.port->available())
    {
      ...
      if (byte == FRSKY_TELEMETRY_START_FRAME) {
        //        hasStuffing = false;  // Regardles of the state restart state machine when start frame found
        state = ORIGIN;
      }
        ...
        id = byte;  // Store the sensor ID, start sarching for data frame
        state = DATA_FRAME;
      }
      else if ((byte == FRSKY_STUFFING) && (state > DATA_FRAME) && (state <= CRC_BYTE)) {
        hasStuffing = true; // Skip stuffing byte in data and mark to xor next byte with 0x20
      }
      else
      {
        if (hasStuffing == true) {
          byte ^= 0x20;  // Xor next byte with 0x20 to remove stuffing
          hasStuffing = false;
        }
        if (state == SENSOR_ID) {
          id = byte;  // Store the sensor ID, start sarching for data frame
        ...
        else if ((state == DATA_FRAME) && ((byte == FRSKY_SENSOR_DATA_FRAME) || (byte == FRSKY_SENSOR_DATA_FRAME)))
        {
          ...
          if (byte == FRSKY_SENSOR_DATA_FRAME) {
            crc = byte;  // If data frame found, initialize the CRC and start collecting APP ID
            state = APP_ID_BYTE_1;
          }
          else {
            hasStuffing = false;  // If empty frame found, skip the frame and start looking for next one
            state = START_FRAME;
          }
        }
        else if (state == APP_ID_BYTE_1) {
          ((uint8_t*)&appId)[0] = byte;  // APP ID first byte collected, look for second byte
          state = APP_ID_BYTE_2;
        }
        else if (state == APP_ID_BYTE_2) {
          ((uint8_t*)&appId)[1] = byte;  // APP ID second byte collected, store APP ID and start looking for DATA
          state = DATA_BYTE_1;
        }
        else if (state == DATA_BYTE_1) {
          ((uint8_t*)&data)[0] = byte;  // DATA first byte collected, look for second byte
          state = DATA_BYTE_2;
        }
        else if (state == DATA_BYTE_2) {
          ((uint8_t*)&data)[1] = byte;  // DATA second byte collected, look for third byte
          state = DATA_BYTE_3;
        }
        else if (state == DATA_BYTE_3) {
          ((uint8_t*)&data)[2] = byte;  // DATA third byte collected, look for fourth byte
          state = DATA_BYTE_4;
        }
        else if (state == DATA_BYTE_4) {
          ((uint8_t*)&data)[3] = byte;  // DATA fourth byte collected, store DATA and look for CRC
          state = CRC_BYTE;
        }
      ...
      { // If OK, send data to registered sensors for decoding and restart the state machine.
        ...
        for (uint8_t i = 0; i < sensorCount; i++) {
            ...
            if (result != SENSOR_NO_DATA_ID) break;
          }
        }
        else {
          hasStuffing = false;
        ...
        // Update CRC value
        if ((state > APP_ID_BYTE_1) && (state <= CRC_BYTE) && (hasStuffing == false)) {
        crc += byte;
        crc += crc >> 8;
        crc &= 0x00ff;
      }
